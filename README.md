...
This implements [MEM-201](https://linear.app/memoji-inc/issue/MEM-201/upload-preview-and-adjust-exposure), [MEM-202](https://linear.app/memoji-inc/issue/MEM-202/build-and-edit-grading-graphs), [MEM-203](https://linear.app/memoji-inc/issue/MEM-203/configure-colour-management-and-cst-nodes), [MEM-204](https://linear.app/memoji-inc/issue/MEM-204/grade-with-cdl-contrast-and-saturation), and [MEM-205](https://linear.app/memoji-inc/issue/MEM-205/support-arri-and-sony-log-workflows). Start with Source → Exposure → Output, then add Exposure, Colour Space Transform (CST), CDL, Contrast, and Saturation nodes. See [grading adjustment modes and domains](docs/grading-adjustments.md) for wheel controls, contrast pivot behavior, and the vibrance response. [MEM-207](https://linear.app/memoji-inc/issue/MEM-207/import-high-bit-depth-png-and-tiff-images) adds precision-preserving 16-bit PNG and TIFF import; see [supported variants and limits](docs/image-import.md). [MEM-214](https://linear.app/memoji-inc/issue/MEM-214/save-restore-and-share-projects) adds local saving and image-free grade links; see [local projects and sharing](docs/projects.md). [MEM-215](https://linear.app/memoji-inc/issue/MEM-215/export-gpu-evaluated-cube-luts) adds **LUT export**: download the current grade as a 17³, 33³ or 65³ Adobe Cube file evaluated by the same GPU program as the preview; see [LUT export](docs/lut-export.md) for file contents, capability probing, tiling and tolerances. [MEM-216](https://linear.app/memoji-inc/issue/MEM-216/measure-and-visualize-lut-fidelity) adds measured LUT error, trilinear/tetrahedral checks and a false-colour overlay; see [LUT fidelity](docs/lut-fidelity.md). [MEM-219](https://linear.app/memoji-inc/issue/MEM-219/support-offline-use) adds **offline use**: after one online load the production build, saved projects and opened or stored samples work without a network, and updates wait for an explicit reload; see [offline use](docs/offline.md) for caching, initial-load requirements and update behaviour. [MEM-220](https://linear.app/memoji-inc/issue/MEM-220/verify-and-package-the-mvp-for-release) adds the integrated release acceptance pass, FFmpeg host verification and the [release record](docs/release-verification.md), including its open blockers. [MEM-233](https://linear.app/memoji-inc/issue/MEM-233/film-inspired-looks-a-pickable-editable-exportable-pool-of-preset) adds **film-inspired looks**: **Browse looks** offers thirteen preset grades that insert into your graph as ordinary editable nodes, with an intensity control, swap, reset and remove, and a look-only LUT export scope; see [film-inspired looks](docs/film-looks.md) for the families, the look space, provenance and the grain and halation exclusion.
...
Vite prints the local address. To build a static distribution, run `npm run build`; output is in `dist`, including the generated offline service worker `sw.js`. `npm run preview` serves that build locally. Offline caching is available only in the built application, not on the dev server.
...
- The preview is reduced to at most 2048 pixels on its long edge. Only this preview is retained by the renderer; uploaded bytes are never sent to a server. Save project stores the original file locally in IndexedDB.
...
Local tests use Chromium with ANGLE SwiftShader. This validates the actual WebGL2 path, including float rendering/readback, but is not a claim of physical-GPU or cross-browser certification. Physical GPUs and drivers, Firefox and Safari remain **open release blockers**, recorded in the [release verification record](docs/release-verification.md).

## What this tool is, and is not

- **Every adjustment depends only on the current pixel's colour.** That is what makes the grade expressible as a 3D LUT. There are no vignettes, power windows, blurs, sharpening or any other operation that reads neighbouring pixels, and there never will be in this tool.
- **Looks are colour response, and nothing else.** The film-inspired looks are tone curves, channel balance and saturation. There is no grain and no halation — not as an omission, but because the only file this tool writes is a `.cube`, and a 3D LUT is a pure function of colour, so neither could ever reach the footage you are grading. The looks are named by family rather than by stock: nothing here is measured against real film.
- **Images stay on your device.** Decoding, grading, scopes, LUT generation and fidelity measurement all run in your browser on your GPU. Nothing is uploaded, there is no account, and a share link carries the graph and its colour tags but never image bytes.
- **A LUT is an approximation, and the tool measures it.** Measure LUT fidelity reports the maximum and 95th-percentile error, in eight-bit code values, between the graded image and the same image through the serialized LUT. An image-based measurement is not a global error bound for every possible colour.
- **Retagging cannot recover range.** Tagging an sRGB JPEG as log reinterprets the values it already has; highlight detail lost to clipping, tone mapping or eight-bit quantization does not come back.
- **Verified hosts.** Exported `.cube` files are checked against FFmpeg's `lut3d` filter in CI, with both trilinear and tetrahedral interpolation. DaVinci Resolve, Photoshop and Lightroom are **not yet verified**; see the release record before making claims about them.
- Desktop browsers with WebGL2 and float rendering only. Mobile and limited-capability devices receive a visible warning instead of a silently degraded result.

## Checks

Nine licensed photographic HDR-source log stills cover skin tones, high-contrast
exteriors, tungsten lighting and a photographed neutral chart. See
[Log sample assets](docs/log-samples.md) for provenance, attribution and reproducible
preparation. `npm run samples:verify` checks file integrity;
`npm run samples:release-check` additionally enforces complete scene/transfer
coverage and passes for this collection.
...
Linux CI installs browser system dependencies with `npx playwright install --with-deps chromium`. Tests run at the agreed public engine and browser-workflow boundaries. They cover analytical exposure values, alpha, pixel orientation, no recompilation on parameter changes, JPEG EXIF orientation, PNG compositing, preview size, numeric entry/reset, malformed-file recovery, and unsupported-device feedback. Offline tests build and serve a production preview on port 4173 (`dist-test`). Graph tests additionally cover topology and enum cache reuse, serial exposure highlight recovery, validation failures, typed connections, internal-edge copy/paste, stable IDs, keyboard history, box selection, snapping, and scrub coalescing. Colour tests cover transfer toes, inverse pairs, gamut primaries, adapted whites, signed/highlight values, CST/output boundary composition, viewer separation, and reversible encoding controls. LUT tests use an independent Cube parser and trilinear applier to check ordering, sizes, tiling, clamp policy, half-float fallback and reproduction tolerances. No real user images are used as fixtures.
