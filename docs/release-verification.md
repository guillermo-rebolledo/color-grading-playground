...
gate, the full browser suite and the FFmpeg host comparison **with `--release`**.
It therefore exits non-zero while the manual host blockers below remain open:
that is the gate working, not a broken command. CI runs the same host check
without `--release`, so a pull request still fails on a real FFmpeg mismatch
while the unverified hosts stay visible in the log and the uploaded evidence. The two release
...
| `host-probe.png`                        | RGB16 probe codes: exact 33³ lattice coordinates, cell centres and off-grid values |
...
spec; this pass proves the parts work together. The downloaded `.cube` is
parsed by the independent reader and checked against the engine's own
evaluation at lattice coordinates, so the file on disk demonstrably carries the
grade the report measured.
...
| Offline                              | Second session against the production preview: a stored sample, a saved grade, offline reload, editing, scopes and `.cube` export, with no other-origin requests | `offline` (update flow, per-sample storage)                 |
...
Recorded automatically in `capability.json` (2026-09-06). [GPU recovery](gpu-recovery.md) describes the same configuration from the recovery side; `capability.json` is the generated source for both:
...
| Lattice points versus float image evaluation      | 1e−5 absolute                         | not established — see blocker 3 | [LUT export](lut-export.md)     |
| Serialized LUT reproduction, off-grid probes      | 0.015 (17³), 0.005 (33³), 0.002 (65³) | not established — see blocker 3 | [LUT export](lut-export.md)     |
| Six-decimal serialization at lattice points       | half a unit in the sixth decimal      | same                            | [LUT export](lut-export.md)     |
| Fidelity versus the independent applier           | 0.005 code values                     | not established — see blocker 3 | [LUT fidelity](lut-fidelity.md) |
...
The only measured RGBA16F number is the 1e−3 capability probe, which
`tests/lut.spec.ts` exercises by failing RGBA32F at the WebGL boundary. No
available device selects the reduced-precision route, so its reproduction and
fidelity tolerances are **not established**; the export panel discloses the
route in use, and a half-float report additionally carries lattice and
error-readback rounding. Neither route proves host-independent precision.
...
with the production serializer. The probe holds exact lattice coordinates, cell
centres and scattered off-grid values, so both the sampling grid and the
interpolation between its points are exercised.
...
- Interpolation: `trilinear` and `tetrahedral` are each compared against their
  own expectation. An identity LUT is reproduced exactly by either method, so
  those two rows agree by construction and test ordering and domain handling
  rather than interpolation. On the graded artifact the two expectations differ
  at 47 of 12,288 samples by up to 1.06 code values, well above the 0.05
  tolerance, so a host quietly using the other method fails.
